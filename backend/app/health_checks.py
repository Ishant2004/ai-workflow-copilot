...
Each check is bounded by a short timeout so a slow/hung dependency can't block the
probe — a replica reports "not ready" rather than hanging, and the orchestrator stops
routing traffic to it (backpressure over collapse). Checks run concurrently.
...
async def check_postgres(database_url: str) -> None:
    ...
    conn = await psycopg.AsyncConnection.connect(dsn, connect_timeout=int(CHECK_TIMEOUT_SECONDS))
    try:
...
async def _run(name: str, coro) -> tuple[str, bool, str | None]:
    try:
...
async def run_readiness_checks(
    *, database_url: str | None, redis_url: str | None
    ...
    tasks = []
    if database_url:
        tasks.append(_run("postgres", check_postgres(database_url)))
    if redis_url:
        tasks.append(_run("redis", check_redis(redis_url)))
